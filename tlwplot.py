 ...
 #   [x, z] = meshgrid(minx:dx: maxx, minz: dz:maxz);
    ...
    for kloop in range(1,len(k)):
      ...
      #  print(kk)
        ...
        print(m,n)
        if ((m+n) == 0): # % check for divide by zero
            r = 9e99;
        else:
            r = (m-n) / (m+n); # % reflection coefficient end;

        R = r * cm.exp(1j*2* m * H);# % reflection     calculation
        A = (1 + r) * (cm.exp((H*n+1j*H*m)) / (1 + R));
        C = 1 / (1 + R);
        D = R*C;
        hs =  np.pi*h0*np.exp(-a * np.abs(kk));
        ht = ht + cm.pi * dk * a * np.exp(-a * np.abs(kk));
